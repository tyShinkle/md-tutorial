...
>:exclamation:<br>Some markup such as headers need to be the first characters of a line. That is because headers are block elements in HTML. When Markdown is converted to HTML, unexpected results may occur if block elements are not the first characters on the line which they appear.
